...
import { User, Phone, Mail, MapPin, BookOpen, Calendar, Building2, Hash, Pencil, X, CheckCircle } from "lucide-react";
...
export default function StudentProfilePage() {
  ...
  if (!profile) {
    return <p className="text-center text-gray-400 py-10">Failed to load profile.</p>;
  }

  const institutionName = profile.institution?.name ?? (profile.institutionId ? "Unknown" : "Online");

  return (
    <div className="space-y-6">
      {/* Profile card */}
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        {/* Avatar + student number banner */}
        <div className="bg-primary px-6 py-6 flex items-center gap-5">
          ...
          <div className="min-w-0">
            ...
            <span className={`inline-block mt-2 text-xs font-semibold px-2.5 py-0.5 rounded-full ${STATUS_STYLES[profile.status] ?? "bg-gray-100 text-gray-600"}`}>
              {profile.status}
            </span>
          </div>
          <button
            onClick={openEdit}
            className="ml-auto flex-shrink-0 flex items-center gap-1.5 bg-white/15 hover:bg-white/25 text-white text-sm px-3 py-1.5 rounded-lg transition-colors"
          >
            <Pencil size={14} />
            Edit
          </button>
