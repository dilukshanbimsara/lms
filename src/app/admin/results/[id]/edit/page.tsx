...
import {
  ...
  AlertCircle, Hash, BookOpen, Loader2,
} from "lucide-react";
...
function EditResultContent() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setYear(sheet.year);
      setExamDate(sheet.examDate ?? "");
      setDescription(sheet.description ?? "");
      setGradeRanges(sheet.gradeRanges);
      setInstitutions(insts);
      const institutionIds = sheet.institutionIds as string[];
      const onlineIncluded = institutionIds.includes("online");
      setSelectedInstitutionIds(institutionIds.filter((x) => x !== "online"));
      setIncludeOnline(onlineIncluded);
      setStudents(
        sheet.results.map((r) => ({
          ...r,
          mark: r.mark ?? "",
  ...
  const toggleInstitution = (instId: string) => {
    setSelectedInstitutionIds((prev) =>
      prev.includes(instId) ? prev.filter((x) => x !== instId) : [...prev, instId]
    );
  };
  ...
  const handleLoadStudents = async () => {
    ...
    try {
      ...
      // Merge: keep existing marks for students already in the list
      const existingMap = new Map(students.map((s) => [s.studentId, s]));
      setStudents(
        data.map((s) => existingMap.get(s.studentId) ?? { ...s, mark: "", grade: "", note: "" })
      );
      setStudentsLoaded(true);
    } catch (e) {
  ...
  const updateGradeRange = (grade: Grade, field: "min" | "max", val: string) => {
    ...
    const newRanges = { ...gradeRanges, [grade]: { ...gradeRanges[grade], [field]: num } };
    setGradeRanges(newRanges);
    setStudents((prev) =>
      prev.map((s) => ({
        ...s,
        grade: s.mark !== "" && s.mark !== null ? computeGrade(s.mark, newRanges) : "",
      }))
    );
  };
    ...
    try {
      ...
      await api.results.update(id, {
        ...
        results: students.map((s) => ({
          ...
          subject: s.subject,
          mark: s.mark !== "" && s.mark !== null ? Number(s.mark) : null,
          grade: s.grade || "",
          note: s.note || '',
        })),
      });
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base font-semibold text-gray-900">Edit Result Sheet</h1>
        </div>
        <div className="flex items-center gap-3">
          {saveError && (
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
          ...
          <p className="text-xs text-gray-400 mb-4 ml-8">Grades are fixed as A, B, C, S, W. Editing ranges will recompute all existing grades.</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
          ...
          <p className="text-xs text-gray-400 mb-4 ml-8">Re-load to add newly registered students. Existing marks are preserved.</p>
              ...
              {institutions.map((inst) => (
                ...
                >
                  ...
                  <div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${
                    ...
                    {selectedInstitutionIds.includes(inst.id) && <CheckCircle size={12} className="text-white" />}
                  </div>
                  {inst.name}
                </label>
              ))}
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-primary/10 text-primary text-sm font-semibold rounded-lg hover:bg-primary/20 transition-colors disabled:opacity-50"
            >
              {loadingStudents ? <Loader2 size={15} className="animate-spin" /> : <Users size={15} />}
              {loadingStudents ? "Loading…" : "Re-load Students"}
            </button>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            {students.length === 0 ? (
              <div className="py-16 text-center text-gray-400">
                ...
                <p className="text-sm">No students in this result sheet.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
